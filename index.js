  ...
  register(api) {
    ...
    api.on('before_prompt_build', async (event) => {
      try {
        ...
        console.error(`[O-Mind] before_prompt_build prompt:`, prompt?.slice(0, 50));
        if (!prompt || prompt.length < 5) return;
        
        const keywords = prompt.slice(0, 100);
        console.error(`[O-Mind] Search keywords:`, keywords);
        
        const result = await callOMind(`/api/memories?q=${encodeURIComponent(keywords)}&limit=10`);
        const memories = result || [];
        console.error(`[O-Mind] Search result count:`, memories.length);
